...
import { type PicturesFetchPicturesResponse, picturesFetchPictures } from "#openapi";
...
document.addEventListener("alpine:init", () => {
  Alpine.data("pictures", (config: AlbumConfig) => ({
    pictures: {} as PicturesFetchPicturesResponse,
      ...
      this.pictures = (
        await picturesFetchPictures({
          query: {
            ...
            album_id: this.config.albumId,
            page: this.page,
            // biome-ignore lint/style/useNamingConvention: API is in snake_case
            page_size: this.config.maxPageSize,
          },
        })
      ).data;
      this.loading = false;
    ...
    nbPages() {
      return Math.ceil(this.pictures.count / this.config.maxPageSize);
    },
  }));
});
